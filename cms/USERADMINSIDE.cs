...
namespace finaluserandstaff
{
    public partial class UserManagementControl : UserControl
    {
        ...
        public UserManagementControl()
        {
            ...
            // Set current user (in real app, this comes from login)
        ...
        private void LoadSampleData()
        {
            ...
            // Sort users by role after loading
        ...
        private void SetupEvents()
        {
            ...
            if (txtSearch != null)
            {
                ...
                // Set initial placeholder text if empty
            ...
            if (dgvUsers != null)
            {
                ...
                dgvUsers.EditingControlShowing += DgvUsers_EditingControlShowing; // Add this line
            }
        ...
        {
            // Don't search when placeholder text is showing
            if (txtSearch.Text == "Search keyword..." || txtSearch.ForeColor == Color.Gray)
            {
                // Show all users when placeholder is active
        ...
        private bool CanModifyUser(UserData targetUser)
        {
            ...
            // SuperAdmin can modify everyone EXCEPT other SuperAdmins
            if (currentRole == UserRole.SuperAdmin)
            {
                // Cannot modify other SuperAdmins
                if (targetRole == UserRole.SuperAdmin && targetUser.ID != currentLoggedInUser.ID)
                {
                    return false;
                }
                return true;
        ...
        private void RefreshUserList()
        {
            ...
            // Smart filtering - searches across ALL columns including date/time
            ...
            if (isSearching)
            {
                filtered = filtered.Where(u =>
                    ...
                    u.LastLogin.ToString("MMMM").ToLower().Contains(searchText) ||  // Month name
                    u.LastLogin.ToString("dddd").ToLower().Contains(searchText) || // Day name
            ...
            // Sort users by role after filtering
        ...
        private void UpdateStatistics()
        {
            ...
            int activeAdmins = displayedUsers.Count(u => u.Status == "ACTIVE" &&
                (u.Role == "ADMIN" || u.Role == "SUPER ADMIN"));
            int inactiveAccounts = displayedUsers.Count(u => u.Status == "INACTIVE");
            int onlineNow = displayedUsers.Count(u => u.Status == "ACTIVE" &&
                u.LastLogin.Date == DateTime.Now.Date);

            // Add space between number and "users"
        ...
        // Make Status column clickable and editable with permission check
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (user != null)
                {
                    ...
                    if (user.Role == "SUPER ADMIN")
                    {
                        ...
                        // Also set selection colors for this role
                ...
                if (dgvUsers.Columns[e.ColumnIndex].Name == "statusColumn")
                {
                    ...
                    if (statusUser != null)
                    {
                        if (statusUser.Status == "ACTIVE")
                        {
                            ...
                            e.CellStyle.SelectionForeColor = Color.Green; // Keep green when selected
                        }
                        else if (statusUser.Status == "INACTIVE")
                        {
                            e.CellStyle.ForeColor = Color.Red;
                            e.CellStyle.Font = new System.Drawing.Font(dgvUsers.Font, FontStyle.Bold);
                            e.CellStyle.SelectionForeColor = Color.Red; // Keep red when selected
                        }

                        // Change selection background to light gray instead of blue
        ...
        // Add this event to color the dropdown items when editing
        private void DgvUsers_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            if (dgvUsers.CurrentCell.ColumnIndex == dgvUsers.Columns["statusColumn"].Index)
            {
                ...
                if (comboBox != null)
                {
                    // Remove blue highlight when selecting
                    ...
                    comboBox.DrawItem += (s, ev) =>
                    {
                        ...
                        // Draw with light gray selection instead of blue
        ...
        // EXPORT BUTTON - Working Export to CSV/Excel
        ...
        // Add this method to sort users by role priority
        ...
        private void btnManageUsers_Click(object sender, EventArgs e)
        {
            ...
            // Refresh your grid after closing
            RefreshUserList();
            UpdateStatistics();
        }
        ...
        public void AddUser(UserData newUser)
        {
            ...
            // Sort users by role after adding
            allUsers = SortUsersByRole(allUsers);

            RefreshUserList();
            UpdateStatistics();

            // Find and highlight the newly added user
            HighlightNewUser(newUser.ID);
        }

        // Add this method to highlight a newly added user
        private void HighlightNewUser(string userId)
        {
            if (dgvUsers == null) return;

            // Find the row with the new user's ID
            for (int i = 0; i < dgvUsers.Rows.Count; i++)
            {
                ...
                if (user != null && user.ID == userId)
                {
                    ...
                    // Flash the row (temporary highlight)
                    FlashRow(i);
                    break;
                }
            }
        ...
        // Add a timer to flash the row (temporary highlight)
        ...
        private void FlashTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                // Alternate between highlight color and normal color
                var row = dgvUsers.Rows[flashRowIndex];
                var user = row.Tag as UserData;

                if (flashCount % 2 == 0)
                {
                    // Highlight color (light gold)
                    row.DefaultCellStyle.BackColor = Color.FromArgb(255, 245, 200);
                }
                else
                {
                    // Restore original color based on role
                    if (user?.Role == "SUPER ADMIN")
                    {
                        row.DefaultCellStyle.BackColor = Color.FromArgb(40, 41, 34);
                    }
